...
Open http://127.0.0.1:8765. The bot and dashboard run while this process and computer remain on. Market observations refresh about every 10 seconds plus API latency; the dashboard polls every 3 seconds. The worker is local, not a hosted 24/7 service.
...
Tests cover a complete paper funding/buy/sell cycle, cost accounting, trend signals, next-candle backtesting, loss guards, locally signed transaction encoding, receipt parsing, confirmation depth, transaction reverts, caps, and duplicate prevention after a broadcast timeout. These are unit/component tests, not a funded mainnet acceptance test.

The displayed backtest uses about 30 days of Kraken BTC/USD data, next-hour opening fills, 0.25% pool fee plus 0.15% adverse slippage per trade, and two estimated gas charges per trade. It starts with $22 cash and excludes the initial BNB conversion cost. It is not an onchain historical replay: historical liquidity, gas, approvals, MEV and BTCB/USDT basis are not reconstructed. Backtest stops are checked hourly; live stops are polled. USD estimates assume USDT is worth $1.
