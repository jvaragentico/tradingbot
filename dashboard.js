...
async function refresh(){
  ...
  try {
    ...
    text('runtime-version','TRADING BOT · V'+(s.version||'1.1'));text('clock',new Date().toLocaleTimeString());text('mode',s.mode+' MODE');
    ...
    text('signal',s.trend_entries===false?'ARB / EXITS':s.signal);text('averages','20h '+usd(s.fast)+' · 50h '+usd(s.slow));text('decision',s.paused?s.pause_reason:s.decision);
